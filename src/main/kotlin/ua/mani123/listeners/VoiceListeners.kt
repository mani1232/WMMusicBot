...
class VoiceListeners(private val discordBot: DiscordBot) : ListenerAdapter() {
    ...
    override fun onGuildVoiceUpdate(event: GuildVoiceUpdateEvent) {
        if ((event.channelLeft != null && event.guild.audioManager.connectedChannel != null && event.channelLeft!!.id == event.guild.audioManager.connectedChannel!!.id && event.channelLeft!!.members.size <= 1) || (event.channelLeft is StageChannel == discordBot.config.autoLeaveFromStage)) {
